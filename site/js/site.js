...
function redirectifnotSession(sock, goal) {
    sock.emit("checkSession")
...
function fetchOnlineUsers(sock) {
    ...
    sock.on("getOnlineUsers", function(users) {
        ...
        $("#online-users").text(s_users)
    })
}

function register(sock, username, password, cpassword) {
    if (password !== cpassword) {
        h_alert("Your passwords don't match!")
        return
    }
    if ((!username) || (!password)) {
        h_alert("Your password or username can't be empty")
        return
    }
    ...
    sock.emit("register", username, hash)
    sock.on("registerSuccess", function() {window.location.replace("/html/login.html")})
    sock.on("registerFail", function() {h_alert("This user already exists!")})
}
...
function login(sock, username, password) {
    ...
    sock.emit("login", hash)
    sock.on("loginSuccess", function() {window.location.replace("/html/chat.html")})
    sock.on("loginFail", function() {h_alert("Wrong password or username!")})
}
